...
TOOL.ClientConVar[ "model" ] = "models/engine/piston_1.mdl"
...
if CLIENT then
    language.Add( "Tool.wire_piston.name", "Piston Tool (Wire)" )
    language.Add( "Tool.wire_piston.desc", "Makes a controllable piston" )
    language.Add( "Tool.wire_piston.0", "Primary: Place piston, Reload: Change all piston paramaters that in hull of target prop" )
...
function TOOL:LeftClick( trace )
	...
	piston = MakeWirePiston(ply, trace.HitPos + (trace.HitNormal * 10), Ang, model, force, sound, length, fx, shouldfix)
...
list.Set( "Pistons", "models/engine/piston_1.mdl", { Flags = "cZ" } ) -- Center And Z is the axis
list.Set( "Pistons", "models/engine/piston02.mdl", { Flags = "cZ" } )
